...
async def _handle_http(path: str, method: str, body: bytes) -> tuple[int, dict, str]:
    logger.debug(f"[HTTP] {method} {path} body_len={len(body)}")
    ...
    if path == "/api/health" and method == "GET":
        ...
        logger.info(f"[HTTP] health → model_loaded={engine.loaded} active={manager.active_count()}")
    ...
    if path == "/api/tts" and method == "POST":
        try:
            ...
            logger.warning("[HTTP] POST /api/tts body 不是合法 JSON")
            return 400, {"Content-Type": "application/json"}, json.dumps({"error": "Invalid JSON"})

        text = req.get("text", "").strip()
        if not text:
            logger.warning("[HTTP] POST /api/tts text 为空")
        ...
        logger.info(
            f"[HTTP] POST /api/tts request_id={request_id} "
            f"text_len={len(text)} speaker={speaker} language={language}"
        )
        ...
        try:
            ...
            logger.exception(f"[HTTP] 合成失败 request_id={request_id}")
            return 500, {"Content-Type": "application/json"}, json.dumps({"error": str(e)})

        elapsed = time.time() - t0
        logger.info(f"[HTTP] 合成完成 request_id={request_id} chunks={len(chunks)} elapsed={elapsed:.2f}s")
        ...
        logger.info(f"[HTTP] 响应 request_id={request_id} chunks={len(results)} resp_size={len(resp_json)} audio_total={total_audio_bytes}")
        return 200, {"Content-Type": "application/json"}, resp_json

    logger.warning(f"[HTTP] 未知路由 {method} {path}")
...
async def _dispatch(reader: asyncio.StreamReader, writer: asyncio.StreamWriter) -> None:
    ...
    logger.debug(f"[Dispatch] 新连接来自 {peer_str}")

    try:
        request_line = await asyncio.wait_for(reader.readline(), timeout=10)
        if not request_line:
            logger.debug(f"[Dispatch] {peer_str} 空请求，关闭")
            writer.close()
            return
        ...
        parts = line.split(" ")
        if len(parts) < 2:
            logger.warning(f"[Dispatch] {peer_str} 非法请求行: {line}")
            writer.close()
            return
        ...
        if is_websocket:
            logger.info(f"[Dispatch] {method} {path} → WebSocket upgrade (peer={peer_str})")
            await _handle_ws_upgrade(reader, writer, request_line, headers, path)
        else:
            logger.info(f"[Dispatch] {method} {path} → HTTP content_length={content_length} (peer={peer_str})")
            body = b""
            if content_length > 0:
                if content_length > 10 * 1024 * 1024:
                    logger.warning(f"[Dispatch] {peer_str} body 过大 ({content_length} bytes)，拒绝")
            ...
            await writer.drain()
            logger.info(f"[Dispatch] {method} {path} → {status} {status_text} resp_body={len(resp_body_bytes)}")

    except (asyncio.TimeoutError, ConnectionResetError, BrokenPipeError):
        logger.debug(f"[Dispatch] {peer_str} 连接中断/超时")
    except Exception as e:
        logger.exception(f"[Dispatch] {peer_str} 异常: {e}")
    finally:
        try:
            writer.close()
        except Exception:
...
async def _handle_ws_upgrade(
    ...
    logger.debug(f"[WS] 创建 WebSocketServerProtocol path={path}")
    ...
    logger.debug(f"[WS] 已注入 {len(raw_data)} bytes HTTP 握手数据")

    try:
        while True:
            data = await asyncio.wait_for(reader.read(65536), timeout=300)
            if not data:
                logger.debug(f"[WS] 对端关闭")
                break
            protocol.data_received(data)
    except asyncio.TimeoutError:
        logger.debug(f"[WS] 超时，关闭连接")
    except (ConnectionResetError, BrokenPipeError):
        logger.debug(f"[WS] 连接中断")
    except Exception:
        pass
...
async def _handle_ws(websocket, path=None) -> None:
    ...
    logger.info(f"[WS] 新连接 session={session.session_id[:8]}… active={manager.active_count()}")

    try:
        async for raw in websocket:
            logger.debug(f"[WS] {session.session_id[:8]}… 收到消息 len={len(raw)}")

            try:
                msg = json.loads(raw)
            except json.JSONDecodeError:
                logger.warning(f"[WS] {session.session_id[:8]}… 非法 JSON，跳过")
                continue

            msg_type = msg.get("type")
            logger.debug(f"[WS] {session.session_id[:8]}… type={msg_type}")
            session.touch()

            if msg_type == "ping":
                logger.debug(f"[WS] {session.session_id[:8]}… ping")
                await _send(websocket, {"type": "pong"})
                continue

            if msg_type == "cancel":
                request_id = msg.get("request_id", "")
                logger.info(f"[WS] {session.session_id[:8]}… cancel request_id={request_id}")
            ...
            if msg_type == "text":
                ...
                if not text:
                    logger.warning(f"[WS] {session.session_id[:8]}… text 消息内容为空")
                    continue
                ...
                logger.info(
                    f"[WS] {session.session_id[:8]}… text "
                    f"request_id={request_id} len={len(text)} "
                ...
                async for chunk in service.synthesize_stream(
                    ...
                    logger.debug(
                        f"[WS] {session.session_id[:8]}… chunk [{chunk.index}] "
                        f"audio={len(audio_b64)} dur={round(duration_ms)}ms last={chunk.is_last}"
                    )
                    ...
                    elapsed = time.time() - t0
                    logger.info(
                        f"[WS] {session.session_id[:8]}… 合成完成 "
                        f"request_id={request_id} chunks={chunk_count} elapsed={elapsed:.2f}s"
                    )
                    await _send(websocket, {
                        "type": "done",
                        "request_id": request_id,
                ...
                logger.warning(f"[WS] {session.session_id[:8]}… 未知消息类型: {msg_type}")
                await _send(websocket, {
                    "type": "error", "message": f"未知消息类型: {msg_type}",
                })

    except websockets.exceptions.ConnectionClosed:
        logger.info(f"[WS] 客户端断开连接 session={session.session_id[:8]}…")
    except Exception as e:
        logger.exception(f"[WS] 处理异常 session={session.session_id[:8]}…: {e}")
        try:
        ...
        logger.info(f"[WS] 会话关闭 session={session.session_id[:8]}… active={manager.active_count()}")
